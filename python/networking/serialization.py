...
class LaserTagJSONDecoder:
    ...
    def _decode(self, element_type, element_value):
        ...
        elif typing.get_origin(element_type) == list and type(element_value) == list:
            # Convert list -> List[T]
            # We should check using typing.get_args(element_type) == type(element_value[i]) for all i but that would be overkill.
            # It is therefore possible that the decoded list's type will not match the expected one.
